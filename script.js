...
fetch('/api/prayer-times')
    ...
    .then((data) => {
        const schedule = data?.schedule || data?.jadwal;
        ...
        // Masukkan waktu ke kartu
        Object.entries(prayerTimes).forEach(([name, time]) => {
            const card = document.querySelector(`#prayer-${name}`);

            if (card && time) {
                const timeElement = card.querySelector('b');
        ...
        // Jalankan countdown setelah data berhasil masuk
        startPrayerCountdown(prayerTimes);
    })
    .catch((error) => {
        console.error('Gagal memuat jadwal sholat:', error);

        document.querySelectorAll('.prayer-item b').forEach((element) => {
            element.textContent = '--:--';
        });
    });
...
function startPrayerCountdown(prayerTimes) {
    ...
    const countdownElement = document.querySelector('#prayer-countdown');
    ...
    function updateCountdown() {
        ...
        for (const prayer of prayers) {
            ...
            const parsed = parseTime(prayerTimes[prayer.key]);
        ...
        // target berikutnya adalah SUBUH besok
        if (!nextPrayer) {

            const subuh = prayers[0];
            const parsed = parseTime(prayerTimes[subuh.key]);

            if (!parsed) return;

            nextPrayer = subuh;

            nextPrayerDate = new Date(now);

            nextPrayerDate.setDate(
                nextPrayerDate.getDate() + 1
            );
        ...
        document
            ...
            .forEach((card) => {
                ...
                const oldBadge = card.querySelector(
                    '.prayer-next-badge'
                );
        ...
        if (nextCard) {
            ...
            badge.className = 'prayer-next-badge';
        ...
        const difference =
            nextPrayerDate.getTime() - now.getTime();
